...
export const SIGNAL_PROTOCOL_STORE_INJECTION_TOKEN =
  'SIGNAL_PROTOCOL_STORE_INJECTION_TOKEN';
...
export class SignalService {
  ...
  async generateManyPreKeys(
    numberOfPreKeys: number = 100
  ...
  async generateSignedPreKeyAndSave(
    identityKeyPair: KeyPairType
  ): Promise<SignedPreKeyPairType> {
  ...
  async register(): Promise<Bundle> {
    ...
    const signedPreKey = await this.generateSignedPreKeyAndSave(
      identityKeyPair
    );
